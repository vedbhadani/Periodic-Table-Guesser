...
function loadNextQuestion() {
    ...
    document.getElementById("timer").textContent = "Time Left: 20s";
    document.getElementById("result").textContent = "";
    timeLeft = 20;

    currentElement = getRandomElement();
    document.getElementById("question-text").textContent = "Guess the element with atomic number:";
    document.getElementById("atomic-number").textContent = currentElement.atomicNumber;

    let answerInput = document.getElementById("answer");
    let answerDisplay = document.getElementById("submitted-answer");
    ...
    setTimeout(() => {
        document.getElementById("hint").textContent = `Hint: Starts with '${currentElement.name.charAt(0)}'`;
    }, 10000);
}
...
function checkAnswer() {
    ...
    answerDisplay.id = "submitted-answer";
    answerDisplay.textContent = `Your Answer: ${userAnswer}`;
    
    answerInput.style.display = "none"; 
    answerInput.insertAdjacentElement("afterend", answerDisplay); 
    ...
    if (userAnswer.toLowerCase() === currentElement.name.toLowerCase()) {
        ...
        document.getElementById("result").textContent = `❌ Wrong! The correct answer was ${currentElement.name}`;
        document.getElementById("score").textContent = `Score: ${score}`;
    }
...
function showAnswer(isCorrect) {
    if (!isCorrect) {
        document.getElementById("result").textContent = `⏳ Game Over! The correct answer was ${currentElement.name}`;
